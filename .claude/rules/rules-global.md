...
Shared code lives in root-level directories: `/components` (com `/components/ui` para primitivos shadcn), `/lib`, `/types`.
Per-route code é colocated dentro de `app/<rota>/`: `_components/`, `_actions/`, `_data-access/` (ver "Estrutura de Páginas" abaixo).
Não há diretório `/src` neste projeto.
...
- app/
  - dashboard/
    - /_data-access # Data Access Layer (DAL) da página
    - /_actions # Server Actions da página
...
- If the component will be reused in multiple pages or modules, place it in the root-level `/components` directory.
  - Only add components to `/components` when you're sure they will be reused broadly.
...
Use Playwright for end-to-end tests (`npm run test -- <arquivo>`, um arquivo por vez).
Aim for high test coverage on critical flows and services.
